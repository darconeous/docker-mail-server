...
function add_amavis () {
  ...
  if ! grep -q "$AMAVIS_CONFIG_HEADER" "$POSTFIX_MAIN_CF"; then
    ...
    echo "content_filter = smtp-amavis:[${AMAVIS_PORT_10024_TCP_ADDR}]:${AMAVIS_PORT_10024_TCP_PORT}" >> "$POSTFIX_MAIN_CF"
  ...
  if ! grep -q "$AMAVIS_CONFIG_HEADER" "$POSTFIX_MASTER_CF"; then
    ...
    echo "  -o mynetworks=172.16.0.0/12" >> $POSTFIX_MASTER_CF
